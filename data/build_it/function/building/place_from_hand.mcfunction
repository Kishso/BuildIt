...
execute as @e[type=minecraft:item_display, tag=BeingPlaced, limit=1] run function build_it:utility/calculate_local_vector
$tag @e[type=minecraft:interaction,nbt={UUID:$(target_project)}] remove CurrentOriginEntity

data modify entity @e[type=minecraft:item_display, tag=BeingPlaced, limit=1] transformation.translation[0] set from storage build_it:vector result.distance_side
data modify entity @e[type=minecraft:item_display, tag=BeingPlaced, limit=1] transformation.translation[1] set from storage build_it:vector result.distance_up
data modify entity @e[type=minecraft:item_display, tag=BeingPlaced, limit=1] transformation.translation[2] set from storage build_it:vector result.distance_forward

$tp @e[type=minecraft:item_display, tag=BeingPlaced, limit=1] @e[type=minecraft:interaction, nbt={UUID:$(target_project)}, limit=1]
