...
$data modify entity @e[type=minecraft:item_display, nbt={UUID:$(cursor_display)}, limit=1] transformation.scale set value [0.9,0.9,0.9]
$execute at @s run tp @e[type=minecraft:item_display, nbt={UUID:$(cursor_display)}] @s
$execute as @e[type=minecraft:item_display, nbt={UUID:$(cursor_display)}] at @s run tp @s ~0 ~0.5 ~0

$tag @e[type=minecraft:item_display, nbt={UUID:$(cursor_display)}, limit=1] add BuildIt_CursorApplyAnimation
