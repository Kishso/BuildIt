...
tellraw @s [{text:"Rotating Project..."}]
...
execute as @e[type=minecraft:interaction, tag=CurrentProjectInteraction, limit=1] at @s rotated as @s run summon minecraft:armor_stand ~ ~ ~ {Tags:["NewOriginEntity"], NoGravity:True, Invisible:True}
execute rotated as @e[type=minecraft:interaction, tag=CurrentProjectInteraction, limit=1] run rotate @e[type=minecraft:armor_stand, tag=NewOriginEntity, limit=1] ~ ~
execute as @e[type=minecraft:armor_stand, tag=NewOriginEntity] at @s rotated as @s run function build_it:block_displays/geometry/macro_relative_rotate with storage build_it:functions rotate_project

tag @e[type=minecraft:interaction, tag=CurrentProjectInteraction] add CurrentOriginEntity
execute as @e[type=minecraft:block_display] at @s if score @s ProjectPiecesTable = @e[type=minecraft:interaction, tag=CurrentProjectInteraction, limit=1] EntityIds run function build_it:reward/helper/rotate_around_origin
execute as @e[type=minecraft:block_display] at @s rotated as @s if score @s ProjectPiecesTable = @e[type=minecraft:interaction, tag=CurrentProjectInteraction, limit=1] EntityIds run function build_it:block_displays/geometry/macro_relative_rotate with storage build_it:functions rotate_project
tag @e[type=minecraft:interaction, tag=CurrentOriginEntity] remove CurrentOriginEntity

kill @e[type=minecraft:armor_stand, tag=NewOriginEntity]
 
# Rotate Origin Itself
execute as @e[type=minecraft:interaction, tag=CurrentProjectInteraction] at @s rotated as @s run function build_it:block_displays/geometry/macro_relative_rotate with storage build_it:functions rotate_project
